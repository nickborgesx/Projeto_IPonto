import flask
from flask import jwt_required, get_jwt_identity, verify_jwt_in_request
...
employees_controller = flask.Blueprint('iponto_controller', __name__)
dao_employees = DAOEmployees()
dao_roles = DAORole()
# module_name = 'iponto'

@iponto_controller.route(f'/api/v1/employees/', methods=['GET'])
@jwt_required()
def get_employees():
    try:
        ...
        dao_employees = DAOEmployees()
        query = f"{DAOEmployees._SELECT_ALL} WHERE {DAOEmployees._COL_NAME} = %s"
        params = (current_user,)
        with dao_employees.connection.cursor() as cursor:
            ...
            if results:
                response = flask.jsonify(response=results)
                response.status_code = 200
            else:
                response = flask.jsonify({'error': 'Nenhum funcionário encontrado.'})
                response.status_code = 400
            return response
    except Exception as e:
        return flask.jsonify({'error': 'Token inválido'}), 400
...
@iponto_controller.route('/api/v1/employeesx/', methods=['GET'])
def get_employeesx():
    ...
    if auth_response.status_code == 200:
        ...
        if "user_info" in response_data:
            ...
            if employees:
                ...
                return flask.jsonify(response=employees_data), 200
            else:
                return flask.jsonify({'error': 'Nenhum funcionário encontrado'}), 400
        else:
            return flask.jsonify({'error': 'Informações do usuário não encontradas na resposta da API de Autenticação'}), 400
    else:
        return flask.jsonify({'error': 'Erro na validação do token JWT'}), auth_response.status_code