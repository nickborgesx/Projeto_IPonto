class SQLEmployees:
    ...
    _CREATE_TABLE = (f'CREATE TABLE IF NOT EXISTS {_TABLE_NAME}'
                     ...
                     f'{_COL_ROLES_ID} INTERGER REFERENCES {_TABLE_NAME_ROLE}({_COL_ID}));')

    _INSERT_INTO = f'INSERT INTO {_TABLE_NAME}({_COL_NAME},{_COL_CPF},{_COL_ROLES_ID}) VALUES (%s, %s,%s) RETURNING id;'
    _SELECT_ALL = f'SELECT m.{_COL_ID}, m.{_COL_NAME}, d.{_TABLE_NAME_ROLE} FROM {_TABLE_NAME} m INNER JOIN {_TABLE_NAME_ROLE} d ON m.{_COL_ROLES_ID} = d.{_COL_ID}'
